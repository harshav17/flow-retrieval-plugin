...
SCHEMA = {
    ...
    "properties": [
        ...
        },
        {
            "name": "description",
            "dataType": ["text"],
            "description": "description that defines the flow being stored",
        },
        {
            "name": "flow",
            "dataType": ["text"],
            "description": "JSON representation of the flow",
        },
        {
            "name": "source",
            "dataType": ["string"],
...
class WeaviateDataStore(DataStore):
    ...
    async def delete(
        ...
        if ids:
            ...
            logger.debug(
                f"Deleting vectors from index {WEAVIATE_INDEX} with ids {ids}")
